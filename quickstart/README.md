...
To log in and use abacus itself, you must create and use *User* accounts in the *tdf* realm of the associated keycloak service.
...
The bootrstrap script also binds this client to a custom, opentdf attribute mapper, which allows the entitlement service to link attributes to given subjects, either clients (NPE subjects that act as services or service providers) or users and services that act on behalf of a single account.


When creating a user, Aud-tdf-attributes


Unique for public access with standard flow, you need this or you will see an error saying ‘audience is not correct’


Redirect and web origins need too e set under client > tdf-attributes > settings


Tif-client is a client-using sdk, the other clients don’t need it


Client has users, can view this

User 2  doesn’t have this (client role -> assigned role)

User 1 can entitle people > 
User 2 can only create attributes > 
