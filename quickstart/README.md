...
See [the opentdf/client readme](https://github.com/opentdf/client-cpp) for details about how to configure a Python or C++ application to use opentdf.
...
Log in using the credentials generated with the bootstrap script (supplied via Helm [here](quickstart/helm/opentdf/charts/bootstrap/values.yaml#L10-L11)), or with the default credentials, which are currently "user1" and the password is "password".
...
Log in with the credentials generated by the [values fed into the bootstrap script](quickstart/helm/opentdf/charts/bootstrap/values.yaml#L10-L11), currently `keycloakadmin`. The corresponding password is found there as well.
